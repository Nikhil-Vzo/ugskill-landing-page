...
export const DashboardMockup: React.FC<DashboardMockupProps> = ({ activeTabOverride }) => {
  ...
  return (
    <div ref={containerRef} className="relative w-full aspect-[16/10] overflow-visible">
      {/* ── All CSS animations in one block — no runtime style injection per render ── */}
      <style>{`
        .mockup-fade-in {
          opacity: 0;
        ...
        /* Scale-based (Y-axis only) — compositor-eligible */
        .wave-bar {
        ...
        /* GPU-composited lift — translate3d + box-shadow only */
        .hover-lift-3d {
        ...
        /* Glow pulse — opacity + scale on compositor */
        ...
        /* Dot pulse — replaces Tailwind animate-pulse to avoid layout thrash */
        @keyframes dotPulse {
          0%, 100% { opacity: 1; }
          50%       { opacity: 0.4; }
        }
      ...
      >
        ...
        >
          ...
          {/* Ambient glows — CSS keyframes, not Framer */}
          ...
          <div
            ...
            style={{ transform: 'translate3d(-15px, -10px, 0px)', transformStyle: 'preserve-3d' }}
          >
                  ...
                  {tabs.map((tab) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-[10px] font-bold transition-colors cursor-pointer ${
                          isActive
              ...
              <div className="flex flex-col gap-2.5 py-1 overflow-hidden" style={{ transform: 'translateZ(10px)' }}>
                ...
                <div className="rounded-xl bg-slate-50 border border-slate-200/50 p-2.5 flex flex-col justify-between h-[95px]">
                  ...
                  <div className="flex items-center justify-center gap-1.5 py-0.5">
                    ...
                    <span className="h-4.5 w-4.5 rounded-full border border-emerald-500 bg-emerald-50 text-[8.5px] font-bold text-emerald-600 flex items-center justify-center shadow-xs">✓</span>
                    <div className="h-0.5 w-5 bg-emerald-400" />
                    {/* Replace Tailwind animate-pulse with custom dot-pulse — avoids layout recalc */}
                    <span className="h-5 w-5 rounded-full bg-emerald-500 text-[8.5px] font-bold text-white flex items-center justify-center shadow-xs dot-pulse">3</span>
          ...
          >
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 select-none">
              ...
              {/* Custom dot-pulse instead of animate-pulse to avoid layout recalc */}
              <div className="h-2 w-2 rounded-full bg-emerald-500 shadow-sm shadow-emerald-200 dot-pulse" />
            </div>
                        ...
                        <div className="flex items-end gap-[1.5px] h-4">
                          {[1, 2, 3.5, 2, 1.5, 3].map((h, idx) => (
                            <span
                              ...
                              style={{ height: `${h * 4}px`, animationDuration: `${0.8 + idx * 0.1}s` }}
                            />
                          ))}
                        </div>
                      </div>
              ...
              <div className="flex flex-col gap-1.5 border border-slate-100 rounded-xl bg-slate-50/50 p-2">
                ...
                <div className="flex flex-col gap-1.5">
                  {jobs.map((job) => (
                    <div
                      key={job.company}
                      className="border border-slate-100/80 rounded-lg bg-white p-2 flex items-center justify-between shadow-xs hover:bg-slate-50/80 hover:-translate-y-0.5 transition-[transform,background-color] duration-200"
                    >
                      <div className="flex items-center gap-2">
          ...
          {/* Floating widget 1 — CSS entry via Framer (one-shot), no infinite JS loop */}
          <motion.div
